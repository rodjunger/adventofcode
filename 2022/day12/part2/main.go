...
func findShortestPath(start, end point, heightMap [][]rune) int {
	...
	var (
		//use a large buffered channel as queue, the size can be calculated based on the number of nodes
...
func main() {
	...
	for i, row := range heightMap {
		for j, letter := range row {
			if letter == 'a' {
				//row, column := i, j
				steps := findShortestPath(point{row: i, column: j}, end, heightMap)
				if steps == 0 {
					continue
				}
